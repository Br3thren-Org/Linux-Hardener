...
# _ssh_any_nonroot_sudo_key — is there a NON-root account that can both log in
# by key AND escalate (member of sudo/wheel)? Required before disabling root
# SSH, or PermitRootLogin=no would strip the last path to privilege.
_ssh_any_nonroot_sudo_key() {
    local sudo_groups=()
    getent group sudo  &>/dev/null && sudo_groups+=("sudo")
    getent group wheel &>/dev/null && sudo_groups+=("wheel")
    [[ ${#sudo_groups[@]} -eq 0 ]] && return 1

    local keys home user grp
    for keys in /home/*/.ssh/authorized_keys; do
        [[ -s "${keys}" ]] || continue
        home="${keys%/.ssh/authorized_keys}"
        user="${home##*/}"
...
ssh_apply() {
    ...
    # --- Lockout guard: disabling root SSH requires a non-root key+sudo
    # account, or the box loses its last path to privileged access ---
    local permit_root="${SSH_PERMIT_ROOT_LOGIN:-prohibit-password}"
    if [[ "${permit_root}" == "no" ]] && ! _ssh_any_nonroot_sudo_key; then
        log_error "ssh_apply: SSH_PERMIT_ROOT_LOGIN=no requested but no non-root account has BOTH an authorized_keys and sudo/wheel membership — keeping 'prohibit-password' to avoid lockout"
