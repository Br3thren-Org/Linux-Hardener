...
# Continue loading on errors — one bad line must not drop the rest of the set
-c
...
RULES
    ...
    local cis_extra_rules
    cis_extra_rules="$(cat <<'RULES'
...
RULES
    ...
    local rules="${minimal_rules}"
    if [[ "${AUDITD_RULES:-minimal}" != "minimal" ]]; then
        rules="${rules}${cis_extra_rules}"
    fi

    # Drop watches on paths that do not exist on this host (tallylog is gone
    # on modern Debian, /etc/sysconfig/* is RHEL-only): auditctl errors on a
    # missing watch target and aborts loading the remaining rules.
    local line watch_path
    while IFS= read -r line; do
        if [[ "${line}" =~ ^-w[[:space:]]+([^[:space:]]+) ]]; then
            watch_path="${BASH_REMATCH[1]}"
            if [[ ! -e "${watch_path}" ]]; then
                printf '# skipped (path not present): %s\n' "${line}"
                continue
            fi
        fi
        printf '%s\n' "${line}"
...
_logging_configure_auditd_conf() {
    ...
    local changed=false
    _logging_set_auditd_key "max_log_file"        "50"     && changed=true
    _logging_set_auditd_key "num_logs"            "5"      && changed=true
    _logging_set_auditd_key "max_log_file_action" "rotate" && changed=true
...
# _logging_set_auditd_key <key> <value> — idempotent auditd.conf setter.
# Returns 0 when the file was changed, 1 when the value was already correct.
_logging_set_auditd_key() {
    local key="${1}" value="${2}"

    if grep -qE "^\s*${key}\s*=\s*${value}\s*$" "${AUDITD_CONF}" 2>/dev/null; then
        return 1
    fi

    backup_file "${AUDITD_CONF}"
    if grep -q "^\s*${key}\s*=" "${AUDITD_CONF}" 2>/dev/null; then
        sed -i "s/^\s*${key}\s*=.*/${key} = ${value}/" "${AUDITD_CONF}"
    else
...
_logging_enable_auditd() {
    ...
    # Do not report success on faith — auditd can refuse to start (containers,
    # broken rules) while every command above is ||-swallowed.
    if svc_is_active auditd; then
        log_success "logging_apply: auditd enabled and running"
    else
        log_error "logging_apply: auditd is NOT active after enable/start attempts — check 'journalctl -u auditd'"
        (( CHANGES_FAILED++ )) || true
        return 1
    fi
}
...
_logging_auditd_pkg_name() {
    case "${DISTRO_FAMILY:-}" in
        ...
        *)
            ...
            log_warn "_logging_auditd_pkg_name: unknown DISTRO_FAMILY '${DISTRO_FAMILY}', defaulting to auditd" >&2
            printf 'auditd'
            ;;
    esac
}
